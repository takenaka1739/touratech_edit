...
useEffect(() => {
  ...
  /**
   * 新規データを API 経由で送信し、登録処理をリクエストする。
   * 
   * @param url - API のエンドポイント
   * @returns 成功時はレスポンス、失敗時は false または undefined
   */
  ...
  /**
   * 編集データを API 経由で送信し、更新処理をリクエストする。
   * 
   * @param url - API のエンドポイント（例: 'items/update/1'）
  ...
  /**
   * 商品分類の登録処理を API 経由でリクエストする。
   * 
   * @param url - API のエンドポイント
   * @param curd - 処理種別（例: 'store'＝新規登録）
   * @returns 保存結果オブジェクト { success: 成否, id: 登録されたカテゴリID }
   */
  ...
  const saveClick: () => void = async () => {
    ...
    if (state.id === undefined) {
      ...
      // variations 配列を整形
      const variations = variChangeItem.map((value: string[]) => ({
        variations1: value[1],
        variations2: value[2],
        variations3: value[3],
        variations4: value[4],
