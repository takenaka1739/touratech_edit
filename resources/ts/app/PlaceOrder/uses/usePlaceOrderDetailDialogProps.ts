...
import { Item, PlaceOrderDetail } from '@/types';
import { appAlert } from '@/components';

/**
 * 発注データ（明細）ダイアログ表示用 hooks
 *
 * @param details
 * @param updateState
 */
export const usePlaceOrderDetailDialogProps = (
  details: PlaceOrderDetail[],
  initialDetailState: PlaceOrderDetail,
  updateDetails: (details: PlaceOrderDetail[], details_amount: number) => void
) => {
  const [state, setState] = useState<PlaceOrderDetail>(initialDetailState);
  ...
  const updateDetailState: <K extends keyof PlaceOrderDetail>(
    props: {
      [key in K]?: PlaceOrderDetail[K];
    }
  ) => void = props => {
  ...
  const sumAmount: (details: PlaceOrderDetail[]) => number = details => {
    return details.reduce((x, y) => {
      return x + toNumber(y.amount ?? 0);
    }, 0);
  };

  const onSelected: (detail: PlaceOrderDetail) => void = detail => {
    if (detail.no) {
  ...
  const sortDetails: (details: PlaceOrderDetail[]) => PlaceOrderDetail[] = details => {
    // details.sort((a, b) => {
