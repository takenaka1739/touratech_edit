...
export type PlaceOrderDetailDialogProps = {
  title: string;
  slug: string;
  isShown: boolean;
  state: PlaceOrderDetail;
  salesTaxRate: number;
  fraction: number;
  updateState: (props: { [key: string]: string | number | undefined }) => void;
  selectedFuncBefore?: (props: Item) => Promise<boolean>;
  onSelected: (detail: PlaceOrderDetail) => void;
  onDeleted: (id: number) => void;
  onCancel: () => void;
};

/**
 * 発注データ（明細）画面 Component
 *
 * @param props
 */
export const PlaceOrderDetailDialog: React.VFC<PlaceOrderDetailDialogProps> = ({
  title,
  slug,
  isShown,
  state,
