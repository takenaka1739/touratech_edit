...
class ConfigUpdateRequest extends BaseRequest
{
  public function rules()
  {
    return [
      ...
      'tel'             => 'required|tel',
      'fax'             => 'required|tel',
      'email'           => 'bail|required|string|email:rfc|max:128',
      'company_level'   => ['required', Rule::in(['A', 'B', 'C', 'D', 'E'])],
      ...
      'tax_rate_change_at'  => 'required|date',

      'send_trader'         => 'bail|nullable|numeric|price',
      'send_personal'       => 'bail|nullable|numeric|price',
  ...
  public function attributes()
  {
    return [
      ...
      'sales_tax_rate'     => '消費税率',
      'pre_tax_rate'       => '変更前税率',
      'tax_rate_change_at' => '税率変更日',
      'send_trader'        => '送料無料判定金額（業者向け）',
      'send_personal'      => '送料無料判定金額（一般向け）',
      'currencies.*.id'    => '掛率',
      'currencies.*.rate'  => '掛率',
      'cods.*.id'          => '代引手数料',
      'cods.*.amount'      => '代引手数料',
