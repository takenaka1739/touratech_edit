...
class SalesPdfService
{
  ...
  /**
   * コントローラ互換メソッド（PDF生成）
   * 既存コードで createPdf() を呼んでいるため、ここで確実に提供する。
   *
   * @param array $data
    ...
    int $max_page)
  {
    ...
    $this->pdf->SetFontSize(12);
    $this->pdf->TextRight(173.13, $base_y + 15.7, (string)$y);
    $this->pdf->SetFontSize(9);
    $this->pdf->Text(173.191, $base_y + 16.821, '年');
    $this->pdf->SetFontSize(12);
    $this->pdf->TextRight(183.547, $base_y + 15.7, (string)$m);
    $this->pdf->SetFontSize(9);
    $this->pdf->Text(184.303, $base_y + 16.821, '月');
    $this->pdf->SetFontSize(12);
    $this->pdf->TextRight(194.84, $base_y + 15.7, (string)$d);
    $this->pdf->SetFontSize(9);
    ...
    foreach ($rows as $row) {
      ...
      $this->pdf->Cell(27.4, 4.35, number_format((float)$row->get('quantity', 0)), 0, 0, "R");
      ...
      $this->pdf->Cell(22.44, 4.35, '（' . number_format((float)$row->get('sales_tax', 0), 0) . '）', 0, 0, "R");
