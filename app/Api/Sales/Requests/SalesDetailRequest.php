...
class SalesDetailRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            // ヘッダ（保存時は必須寄せ）
            // =========================
            'sales_at'        => ['required', 'string'],   // 日付型で厳密にするなら 'date' 等に変更
            'customer_id'     => ['required', 'integer'],
            'corporate_class' => ['required', 'integer'],  // 画面の「支払方法」に対応
            'send_flg'        => ['sometimes', 'boolean'],

            // 発送あり(send_flg=true) の場合に必須
            'name'            => ['required_if:send_flg,true', 'nullable', 'string'],
            'zip_code'        => ['required_if:send_flg,true', 'nullable', 'string'],
            'address1'        => ['required_if:send_flg,true', 'nullable', 'string'],
            'address2'        => ['sometimes', 'nullable', 'string'],
            'tel'             => ['required_if:send_flg,true', 'nullable', 'string'],
    ...
    {
        return [
            'sales_at.required'            => ':attributeは必ず指定してください。',
            'customer_id.required'         => ':attributeは必ず指定してください。',
            'corporate_class.required'     => ':attributeは必ず指定してください。',

            'name.required_if'             => '発送を指定した場合、:attributeは必ず指定してください。',
            'zip_code.required_if'         => '発送を指定した場合、:attributeは必ず指定してください。',
            'address1.required_if'         => '発送を指定した場合、:attributeは必ず指定してください。',
            'tel.required_if'              => '発送を指定した場合、:attributeは必ず指定してください。',

            'details.required'             => ':attributeは必ず指定してください。',
            'details.min'                  => ':attributeは必ず指定してください。',
            'details.array'                => ':attributeの形式が不正です。',
            'details.*.no.min'             => ':attributeは1以上で入力してください。',
            'details.*.quantity.required'  => ':attributeは必ず指定してください。',
            'details.*.quantity.min'       => ':attributeは1以上で入力してください。',
            'details.*.unit_price.required'=> ':attributeは必ず指定してください。',
        ];
    }
}
