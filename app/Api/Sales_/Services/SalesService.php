...
class SalesService
{
  ...
  public function get_by_receive_id(int $receive_order_id)
  {
    ...
    foreach ($details as $d)
    {
      ...
      if ($d->quantity > $d->domestic_stocks) {
        $d->quantity = $d->domestic_stocks;
      }
    ...
    return [
      'sales_at' => Carbon::today()->format('Y/m/d'),
  ...
  {
    $m = new Sales();
    $m->sales_at = Carbon::today()->format('Y/m/d');
  ...
  public function getPdfData(array $input)
  {
    ...
    $data["sales_tax_rate"] = $configs->getSalesTaxRate($input->get('sales_at'));
  ...
  private function setCondition($query, array $cond)
  {
    ...
    if ($c_sales_date_from) {
      $query->where('sales_at', '>=', $c_sales_date_from);
    }

    $c_sales_date_to = $cond->get('c_sales_date_to');
    if ($c_sales_date_to) {
      $query->where('sales_at', '<=', $c_sales_date_to);
    }
